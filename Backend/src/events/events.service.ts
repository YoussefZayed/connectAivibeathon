...
export class EventsService {
  ...
  async crawlAndSaveEvents(url?: string, instructions?: string) {
    ...
    try {
      ...
      const crawlUrl =
        url ||
        'https://www.ctvnews.ca/ottawa/article/whats-happening-in-ottawa-this-weekend-july-4-6/';
      const crawlInstructions =
        instructions || 'Find all things to do in ottawa this weekend';

      // Call Tavily API to crawl the website
      const tavilyResponse = await this.crawlWithTavily(
        crawlUrl,
        crawlInstructions,
      );
      ...
      if (formattedEvents.length > 0) {
        ...
        this.logger.info(
          `Successfully saved ${formattedEvents.length} events to database`,
        );
      }
  ...
  private async crawlWithTavily(url: string, instructions: string) {
    ...
    try {
      ...
      if (isTimeout || isRateLimit) {
        if (isTimeout) {
          this.logger.warn(
            'Tavily API request timed out, attempting to use cached log data...',
          );
        } else {
          this.logger.warn(
            'Tavily API rate limit reached (429), attempting to use cached log data...',
          );
        }
        ...
        if (logData) {
          this.logger.info(
            'Using cached log data from previous successful request',
          );
          return logData;
        }
      }
      ...
      throw new Error(
        `Tavily API error: ${error.response?.status} ${error.response?.statusText} - ${error.response?.data?.message || error.message}`,
      );
    }
  }
  ...
  private getLatestLogData(): any | null {
    try {
      ...
      const files = readdirSync(logsDir)
        .filter(
          (file) =>
            file.startsWith('tavily-response-') && file.endsWith('.json'),
        )
        .sort()
...
${JSON.stringify(crawlData, null, 2)}
      ...
      const formattedEvents: EventData[] = events.map((event: any) => ({
        ...
        image_url:
          event.image_url ||
          'https://via.placeholder.com/400x300?text=Ottawa+Event',
      }));
