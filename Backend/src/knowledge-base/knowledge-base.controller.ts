...
export class KnowledgeBaseController {
  ...
  async createKnowledgeBaseEntry() {
    return tsRestHandler(
      knowledgeBaseContract.createKnowledgeBaseEntry,
      async ({ body }) => {
        try {
          const result =
            await this.knowledgeBaseService.createKnowledgeBaseEntry(body);
          return {
            status: 201,
            body: result,
          };
  ...
  async getKnowledgeBaseEntries() {
    return tsRestHandler(
      knowledgeBaseContract.getKnowledgeBaseEntries,
      async ({ params }) => {
        try {
          const entries =
            await this.knowledgeBaseService.getKnowledgeBaseEntries(
              params.userId,
            );
          return {
            status: 200,
  ...
  async getKnowledgeBaseEntry() {
    return tsRestHandler(
      knowledgeBaseContract.getKnowledgeBaseEntry,
      async ({ params }) => {
        try {
          const entry = await this.knowledgeBaseService.getKnowledgeBaseEntry(
            params.id,
          );
  ...
  async queryKnowledge() {
    return tsRestHandler(
      knowledgeBaseContract.queryKnowledge,
      async ({ body }) => {
        try {
