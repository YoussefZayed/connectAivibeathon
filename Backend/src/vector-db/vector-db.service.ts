...
export class VectorDbService implements OnModuleInit {
  ...
  async indexAllKnowledgeBaseEntries() {
    try {
      ...
      this.logger.log(
        `Indexed ${entries.length} knowledge base entries into the vector database`,
      );
      ...
      this.logger.error(
        `Error indexing knowledge base entries: ${error.message}`,
      );
      throw new Error(
        `Failed to index knowledge base entries: ${error.message}`,
      );
    }
  }
  ...
  async storeKnowledgeBaseEntry(entry: {
    ...
    try {
      ...
      this.logger.log(
        `Stored knowledge base entry ${entry.id} for user ${entry.userId}`,
      );
